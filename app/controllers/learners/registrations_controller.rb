class Learners::RegistrationsController < Devise::RegistrationsController
  ...
  devise_parameter_sanitizer.for(:account_update) { |u| u.permit(:email, :firs_name, :last_name, :date_of_birth, :phone, :address, :city, :zip, :country, :password, :picture, :current_password) }
  end
